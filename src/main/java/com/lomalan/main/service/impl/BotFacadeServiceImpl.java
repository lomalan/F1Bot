...
public class BotFacadeServiceImpl implements BotFacadeService {
    ...
    public PartialBotApiMethod<Message> processUpdateWithMessage(Update update) {
        ...
        return sendBotApiMethod(update, getMessageIfNotEmpty(processMessageServices(update, telegramUser)),
                telegramUser);
    }

    private MessageContainer getMessageIfNotEmpty(Optional<MessageContainer> optionalMessageContainer) {
        return optionalMessageContainer.orElse(new MessageContainer("Please use the main menu"));
    }
    ...
    private Optional<MessageContainer> processMessageServices(Update update, TelegramUser user) {
        return messageServices.stream().map(messageService -> messageService.processMessage(update, user))
                .flatMap(Optional::stream).findFirst();
    }

    private PartialBotApiMethod<Message> sendBotApiMethod(Update update, MessageContainer container,
            TelegramUser user) {
        return container.getPhoto() != null ? sendPhoto(update, container, user) : sendMessage(update, container, user);
    }

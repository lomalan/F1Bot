...
public class LiveTimingService {
    ...
    private boolean isInvalidDateForLiveTiming(Race nextRace) {
        return nextRace.isInvalidDateForLiveTimingPosting() && nextRace.getQuali().isInvalidDateForLiveTimingPosting()
    ...
    private void executeDataForSubscribers() {
        ...
        allUsers.stream().filter(TelegramUser::isSubscribedOnLiveUpdates).forEach(this::executeMessage);
    }

...
public class WeatherService {
    ...
    private void executeMessage(Location location, WeatherResponse weather, TelegramUser user) {
        messageExecutor.executeMessage(user,
                MessageConstructor.constructWeatherMessage(weather, location.getLocality()));
    }

    private boolean isInvalidDateForWeatherPosting(Race nextRace) {
        return nextRace.isInvalidDateForWeatherPosting() && nextRace.getQuali().isInvalidDateForWeatherPosting()
                && (nextRace.getSprint() == null || nextRace.getSprint().isInvalidDateForWeatherPosting());
    }

    private List<TelegramUser> getWeatherSubscribers() {
        return userRepository.findAll().stream().filter(TelegramUser::isSubscribedOnWeather)

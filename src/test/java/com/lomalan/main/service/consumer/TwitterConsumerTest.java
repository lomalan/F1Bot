...
class TwitterConsumerTest {
    ...
    void whenThereAreNoUsersSubscribeOnTwitterUpdatesThenNotExecuteMessages() {
        when(telegramUserRepository.findAll()).thenReturn(Collections.singletonList(
    ...
    void whenThereAreUsersSubscribeOnTwitterUpdatesThenExecuteMessages() {
        when(telegramUserRepository.findAll()).thenReturn(Collections.singletonList(TelegramUser.builder()
                .subscribedOnTwitterUpdates(true).build()));
        testObject.consume("url");

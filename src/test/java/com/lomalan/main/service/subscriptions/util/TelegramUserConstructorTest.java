...
public class TelegramUserConstructorTest {
    ...
    public void whenCallingMethodThenReceivingResult() {
        ...
        assertEquals(
                TelegramUser.builder().userName("name").firstName("firstName").lastName("lastName").chatId("111")
                        .isBot(false).state(BotState.MAIN_MENU).build(),
                TelegramUserConstructor.prepareUserToSave(user, botState));
    }
}
